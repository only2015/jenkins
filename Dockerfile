...
RUN tar zxf /usr/java/va/jdk-8u121-linux-x64.tar.gz -C  /usr/java && \
    cd /usr/java/ && \
    mv jdk1.8.0_121 jdk && \
...
RUN wget et http://mirrors.tuna.tsinghua.edu.cn/apache/maven/maven-3/3.3.9/binaries/apache-maven-3.3.9-bin.tar.gz -P  /tmp
...
RUN wget  http://archive.apache.org/dist/tomcat/tomcat-7/v7.0.69/bin/apache-tomcat-7.0.69.tar.gz -P  /tmp

volume /data/jenkins

RUN cd /tmp && \
    tar xzf zf apache-tomcat-7.0.69.tar.gz -C   ${JOB_HOME} && \
